...
[task_local]
#苹果抽奖机
10 0 * * * https://raw.githubusercontent.com/jackpater/MyActions/main/scripts/jd_apple_live.js, tag=苹果抽奖机, enabled=true

================Loon==============
[Script]
cron "10 0 * * *" script-path=https://raw.githubusercontent.com/jackpater/MyActions/main/scripts/jd_apple_live.js,tag=苹果抽奖机

===============Surge=================
苹果抽奖机 = type=cron,cronexp="10 0 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/jackpater/MyActions/main/scripts/jd_apple_live.js

============小火箭=========
苹果抽奖机 = type=cron,script-path=https://raw.githubusercontent.com/jackpater/MyActions/main/scripts/jd_apple_live.js, cronexpr="10 0 * * *", timeout=200, enable=true
 */
const $ = new Env('苹果抽奖机');
...
function jdapple_collectScore(taskToken, taskId, itemId, actionType=0) {
  return new Promise(resolve => {
    let body = { "appId":"1EFRTxw","taskToken":taskToken,"taskId":taskId,"itemId":itemId,"actionType":actionType }
...
function jdapple_getTaskDetail(get=1) {
  return new Promise(resolve => {
    $.post(taskPostUrl("healthyDay_getHomeData", {"appId":"1EFRTxw","taskToken":""}, ), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
